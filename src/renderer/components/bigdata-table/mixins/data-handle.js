...
export default {
	...
	methods: {
		...
		//  获取高度与数量
		updateHeight() {
			this.itemNum = Math.ceil((Number(this.height) || 0) / this.rowHeight) + this.appendNum
		...
		initGroupHeight(data) {
			//  分组数据
			let moduleNb = Math.ceil((Number(this.height) || 0) / this.rowHeight) + this.appendNum
			...
			this.groupIndex = 0
			if (this.$refs.bodyWrapper) this.$refs.bodyWrapper.scrollTo(0, 0)
			return groupHeight
		},
		//  设置顶部定位
		setTopPlace() {
